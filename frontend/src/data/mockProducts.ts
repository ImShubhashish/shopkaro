...
export const CATEGORIES_LIST: CategoryDef[] = [
  ...
  { id: 'toys', name: 'Toys & Kids', slug: 'toys' },
  { id: 'health', name: 'Health & Care', slug: 'health' },
  { id: 'furniture', name: 'Furniture', slug: 'furniture' },
  { id: 'books', name: 'Books', slug: 'books' },
  { id: '2wheelers', name: '2 Wheelers', slug: '2wheelers' },
];

export const getUniqueProductImage = (productId: string, name: string, categoryId: string): string => {
  const cleanTitle = encodeURIComponent(name.split('(')[0].trim());

  const categoryKeywordMap: Record<string, string> = {
    fashion: 'apparel,footwear,fashion,sneakers',
    mobiles: 'smartphone,mobile,phone',
    electronics: 'gadget,tech,electronics,headphones',
    beauty: 'cosmetics,skincare,makeup,beauty',
    home: 'homedecor,kitchen,coffee,cozy',
    appliances: 'appliance,television,washingmachine,ac',
    toys: 'toy,lego,actionfigure,boardgame',
    health: 'fitness,wellness,health,gym',
    furniture: 'furniture,chair,sofa,table',
    books: 'book,novel,reading,hardcover',
    '2wheelers': 'motorcycle,scooter,electricscooter,bike',
  };

  const keywords = categoryKeywordMap[categoryId] || 'product';
  return `https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&w=800&q=80&sig=${productId}&keywords=${keywords}&title=${cleanTitle}`;
};
...
// Carefully selected unique Unsplash image URL for every single product (220 unique images)
const categoryUniqueImages: Record<string, string[]> = {
  'fashion': [
...
export const generate20ProductsPerCategory = (): Product[] => {
  const allProducts: Product[] = [];
  let globalCount = 1;

  for (const catDef of CATEGORIES_LIST) {
    const names = categoryProductNames[catDef.id] || [];
    const images = categoryUniqueImages[catDef.id] || categoryUniqueImages['electronics'];

    for (let i = 0; i < 20; i++) {
      const name = names[i] || `${catDef.name} Product Model #${i + 1}`;
      const img = images[i % images.length];
      const basePrice = Math.round((Math.sin(globalCount) * 0.4 + 0.6) * 50000 + 499);
      const rating = Number((4.0 + (globalCount % 10) * 0.1).toFixed(1));
      const stock = (globalCount * 3) % 25 + 2;
      const numReviews = (globalCount * 17) % 400 + 12;
      ...
      allProducts.push({
        ...
        category: { id: catDef.id, name: catDef.name, slug: catDef.slug },
      });
...
export const mockProducts: Product[] = generate20ProductsPerCategory();
